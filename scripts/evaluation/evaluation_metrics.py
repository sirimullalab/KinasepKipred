...
def AUC(y, f):
    """
    Task:    To compute area under the ROC curve (AUC) given as
             interaction threshold value [7.55] (for Metz data set)
             to binarize pKd's into true class labels.
    ...
    y_binary = copy.deepcopy(y)
    y_binary = preprocessing.binarize(
        y_binary.reshape(1, -1), threshold=thr, copy=False)[0]
    fpr, tpr, thresholds = metrics.roc_curve(y_binary, f, pos_label=1)
