...
class Model_Evaluation:
    ...
    def model_load(self):
        if self.model == 'rfr':
            model_rfr = joblib.load('/Users/gvin/pki_paper_work/pki_random_forest/rfr_600.mdl')
            if self.data_set == 'test':
                x_test = np.load('/Users/gvin/pki_paper_work/pki_random_forest/x_rfr_600.npy')
                y_test_rfr = np.load('/Users/gvin/pki_paper_work/pki_random_forest/y_rfr_600.npy')
                y_pred = model_rfr.predict(x_test)
                return y_pred
            if self.data_set == 'external':
                data = np.load('/Users/gvin/pki_paper_work/idg_github/total_metz_pki_propy.npy')
                x_test = data[:,:-1]
                y_test_ext = data[:,-1]
                y_pred = model_rfr.predict(x_test)
                return y_pred
        else:
            model_xgb = joblib.load('/Users/gvin/pki_paper_work/xgboost_new/pki_xgboost/xgb_new_best.mdl')
            if self.data_set == 'test':
                x_test = np.load('/Users/gvin/pki_paper_work/xgboost_new/pki_xgboost/x_xgb_cv.npy')
                y_test_xgb = np.load('/Users/gvin/pki_paper_work/xgboost_new/pki_xgboost/y_xgb_cv.npy')
