...
import pandas as pd
from metk_util import rmse, mean_absolute_error, pearson_confidence,spearman_confidence,max_possible_correlation, kcal_to_ki
...
def metk_report(df_kcal):
    ...
    pearson_vals = [pearson_r] + list(pearson_confidence(pearson_r, N))
    ...
    report.append("Pearson = %0.2f  95%%CI = %.2f %.2f" % tuple(pearson_vals))
    report.append("Spearman = %0.2f 95%%CI = %.2f %.2f" % tuple(spearman_vals))
